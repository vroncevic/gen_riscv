...
Info
    Engine orchestrating the initialization and execution of gen_riscv.
'''
...
class GenRiscv(Base):
    '''
        ...
        It defines:
            ...
            :attributes:
                | _is_initialized - The flag indicating whether the gen_riscv engine is initialized.
            ...
            :methods:
                | __init__ - Initializes the gen_riscv engine with adapters and services.
                | process - Processes the gen_riscv commands.
    '''
    ...
    def __init__(self, bundle: GenRiscvBundle) -> None:
        '''
            Initializes the gen_riscv engine with adapters and services.

            :param bundle: gen_riscv bundle containing adapters and services.
            :exceptions: None.
        '''
        ...
        try:
            ...
            self._logger.write_log(INFO, '✅ gen_riscv: engine initialized successfully!')

        except (ATSValueError, ATSTypeError) as exc:
            stdout.write(f'❌ gen_riscv: {exc}!\n')

        except Exception as exc:
            stdout.write(f'❌ gen_riscv unexpected exception: {exc}!\n')

    def process(self, verbose: bool = False) -> bool:
        '''
            Processes the gen_riscv commands.
        ...
        try:
            if self.is_initialized():
                ...
                if result.get("returncode") != 0:
                    self._logger.write_log(ERROR, f'❌ gen_riscv: {result.get("stderr") or "failed!"}')
                    return False

                self._logger.write_log(INFO, '✅ gen_riscv: done!')
                self._logger.write_log(INFO, '✅ gen_riscv: exiting successfully!')
                return True

            self._logger.write_log(ERROR, '❌ gen_riscv: engine not initialized!')
            return False

        except (ATSValueError, ATSTypeError) as exc:
            self._logger.write_log(ERROR, f'❌ gen_riscv: {exc}!')
            return False

        except Exception as exc:
            self._logger.write_log(ERROR, f'❌ gen_riscv unexpected exception: {exc}!')
            return False
